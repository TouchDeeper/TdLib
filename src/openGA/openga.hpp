...
#ifndef TDLIB_OPPENGA_HPP
#define TDLIB_OPPENGA_HPP
#include <vector>
#include <random>
#include <chrono>
#include <thread>
...
namespace EA {
    ...
    public:
                ...
                if (!reference_vector_divisions) {
                    ...
                    if (N_robj == 1)
                        throw std::runtime_error(
                                "The length of objective vector is 1 in a multi-objective optimization");
        ...
        StopReason solve_next_generation() {
            ...
            transfer(new_generation);//copy all the chromosome to the new_generation
            crossover_and_mutation(new_generation);

            finalize_objectives(new_generation);
            rank_population(new_generation);  // set the chance of each chromosome used for selection
            ...
            rank_population(new_generation); // used for elite transfer, crossover and mutation
		...
		*/
        ...
        void select_population_SO(const thisGenerationType &g, thisGenerationType &g2) {
            ...
            for (int i = 0; i < int(population) - elite_count; i++) {
                int j;
                bool allowed;
                //check if the chromosome selected is the elite has been transferd
                do {
                    allowed = true;
                    j = select_parent(g);
        ...
        void crossover_and_mutation(thisGenerationType &new_generation) {
            ...
            if (!multi_threading || N_threads == 1 || is_interactive()) {
                ...
                for (unsigned int i = 0; i < N_add; i++)//the new choromosome plus the last chromosome = new generation
